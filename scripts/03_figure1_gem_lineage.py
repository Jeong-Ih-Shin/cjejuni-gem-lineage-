...
def main():
    ...
    NO_ELLIPSE = {'ST-658 complex'}  # documented split structure

    fig, axes = plt.subplots(1, 2, figsize=(6.69, 3.5), gridspec_kw={'width_ratios': [1.5, 1]})
    plt.subplots_adjust(top=0.88, bottom=0.13, left=0.02, right=0.96, wspace=0.5)
    ...
    for i, this_cc in enumerate(top_ccs):
        ...
        ax.scatter(sub[:, 0], sub[:, 1], c=[CC_COLORS[i]],
                   label=f'{this_cc.replace(" complex","")} (n={n})',
    ...
    ax.set_title('A', loc='left', x=-0.10, fontsize=11, fontweight='bold')
    ...
    for this_cc in cc_counts[cc_counts >= 2].index:
        ...
        per_cc.append({'cc': f'{this_cc.replace(" complex","")} (n={len(in_idx)})',
