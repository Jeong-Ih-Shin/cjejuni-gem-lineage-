...
def bootstrap_ci(in_mat, out_mat, n_boot=2000, seed=42, metric='jaccard'):
    """Isolate-level bootstrap 95% CI for mean within-/between-CC distance.

    Isolates (not pairs) are resampled with replacement, because pairwise
    distances are not independent.
    """
...
def main():
    ...
    for this_cc in cc_counts[cc_counts >= 2].index:
        ...
        wlo, whi, blo, bhi = bootstrap_ci(in_mat, out_mat, metric='jaccard')
