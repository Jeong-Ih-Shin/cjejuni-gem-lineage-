"""
Step 05 - Figure 3: Lineage-associated reactions (CC-443 depleted vs CC-658 enriched).
...
def main():
    ...
    print(f'CC-443 depleted reactions (sig): {len(st443_sig)}')
    print(f'CC-658 enriched reactions (sig): {len(st658_sig)}')
    ...
    print('\nCC-443:'); print(agg443.to_string(index=False))
    print('\nCC-658:'); print(agg658.to_string(index=False))
    ...
    ax.set_xlabel('Number of reactions depleted in CC-443\n(vs all other isolates)')
    ...
    # B - CC-658 enriched, with sulfur respiration highlight
    ax = axes[1]
    ax.grid(axis='x', alpha=0.5, color=SEM['grid'], linewidth=0.5)
    ...
    ax.set_xlabel('Number of reactions enriched in CC-658\n(vs all other isolates)')
